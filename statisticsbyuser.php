...
$(document).ready(function() {
  ...
  <div class="container">
      ...
      <ul class="nav nav-tabs" >
        <li class="active"><a data-toggle="tab" href="#home">Passés</a></li>
        <li><a data-toggle="tab" href="#menu1">Annulés</a></li>
      </ul>
 ...
 <div class="tab-content">
   <div id="home" class="tab-pane fade in active">
     <br>
   <h2 align="center"> Les évenements passés : </h2>

   <hr width="15%" style="border-top-color: #a5b1c2;">
   <br>
   <table id="table" class=" display row-border  " style="width:100%">
     <thead>
       <tr>
         <th style="text-align:center">Id</th>
         <th style="text-align:center">Nom de l'événement</th>
     ...
     <tbody>
       <?php
       function statisticsbyuser($iduser)
       {
       ...
       if (!$conn) // connexion échouée
        {
       ...
       else //  connexion établie
         {
           // récuperer l'id de l'utilisateur
             ...
             // $id=iduser($_POST["user"]);
             // echo " l'id est a ",$iduser;
         $cmd="SELECT `id_res`, `id_user`, `date`, `h_deb`, `h_fin`, `an_resp`,`matricule_resp`, `description`,`nom_event` FROM `reservation` WHERE '$today'>`date` AND id_user='$iduser' AND accord=1 AND accept=1 AND annul<>1";
           ...
           while($row = mysqli_fetch_assoc($result))
            {
              ...
              echo "<tr> <td>".$id_res."</td> <td>".$nom_event."</td> <td>".$user."</td> <td>".$date."</td> <td>".$hd."-".$hf."</td> <td>".$descrp."</td> </tr>";
       echo '  ';
            }
            echo    "</tbody>";
            echo"</table>";
            ...
            <br>
            <h2 align="center">Les évenements annulés</h2>
            <hr width="15%" style="border-top-color: #a5b1c2;">
            <br>
            <table id="table1" class="display row-border" style="width:100%">
            <thead>
            <tr>
            <th style="text-align:center">Id</th>
            <th style="text-align:center">Nom de l\'événement</th>
            ...
            $cmd2="SELECT `id_res`, `id_user`, `date`, `h_deb`, `h_fin`, `an_resp`,`matricule_resp`, `description`,`nom_event` FROM `reservation` WHERE '$today'>`date` AND id_user='$iduser' AND accord=1 AND accept=1 AND annul=1";
                     ...
                     if (!$conn) // connexion échouée
                      {
                       ...
                       { // récuperer l'is de cette salle
                       $cmd="SELECT `id_user` FROM `user` WHERE nom_user='$name'";
                         $result = mysqli_query($conn, $cmd);
                         while($row = mysqli_fetch_assoc($result))
                          {
                            $un=$row["id_user"];
